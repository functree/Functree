...
#### 4. 指针(Pointer)
**单项指针**仅指向1个单独的变量。  
**单项指针**语法：`*T`，**解引用**获取指针对应内容的语法：`ptr.*`；取变量地址的语法：`&x`：
 ```
...
**静态局部变量**是指在函数或块作用域内声明的struct、枚举等**聚合类型**变量。  
**静态局部变量**有静态生命周期，其作用域属于**函数作用域**或**块作用域**：
 ```
...
**容器级变量**是指在**功件文件**（功件文件也是容器）中顶级声明的变量，其具有静态生命周期，其作用域属于**功件作用域**。  
如果声明**容器级变量**时，设置了初始值，则其值为**编译时已知**，否则为**运行时已知**：
 ```
var y: i32 = add(10, x);
...
语句块用来限制声明变量的**作用域**，语句块内部声明的变量，在语句块外部无法使用，下列测试无法通过：
 ```
...
test "optional type" {
 ...
 ```
指针不可以设为 `null`，但是**可选类型指针**能够设为 `null`。  
**可选类型指针**的语法：`?*T`，使用 `ptr.?.*` 获取**可选类型指针**的对应内容:
 ```
